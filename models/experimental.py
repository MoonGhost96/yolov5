...
class CSSum(nn.Module):
    def __init__(self, c, reduction=4):
        ...
        # channel
        self.Squeeze = nn.AdaptiveAvgPool2d(1)
        self.Excitation = nn.Sequential(nn.Conv2d(2 * c, c // reduction, kernel_size=1),
                                        nn.ReLU(),
    ...
    def forward(self, x):
        ...
        # spatial
        avg_f = torch.mean(fusion, dim=1, keepdim=True)
        max_f, _ = torch.max(fusion, dim=1, keepdim=True)
        w2 = torch.cat([avg_f, max_f], dim=1)
        w2 = self.sigmoid(self.conv3(self.conv2(self.conv1(w2))))
